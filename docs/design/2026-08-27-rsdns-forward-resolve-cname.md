...
- **应答已自带最终地址**（链末 CNAME 的 target 与应答最后一个非 CNAME 记录
  的 owner 相同，且该记录是 A/AAAA，如 `www CNAME cdn, cdn A 1.2.3.4`）→
  **直接折叠**：删除所有 CNAME，仅保留 A/AAAA 记录，且这些记录的 owner name
  **改写为原查询名**，不发起二次查询。
...
| 应答已自带完整链 | 删除所有 CNAME，A/AAAA owner 改写为原查询名，不发起二次查询 |
...
- 新增 `resolve_cnames` 方法：当应答首条为 CNAME 且连续多条都是 CNAME 时，
  ...
  - 完整链判定：`chain_completed_with_address(answers)` → 链末 CNAME 的 target
    与最后一个非 CNAME 记录（须为 A/AAAA）的 owner 相同；
  - 折叠函数：`collapse_completed_chain(resp, owner)` → 删除 CNAME 链，
    把剩余 A/AAAA 的 owner 改写为原查询名；
...
- 纯函数单测：分类（空 / A / AAAA / CNAME / 其他）、A/AAAA 过滤 + owner 改写、
  完整链判定（相同 / 不同 / 末尾非 A/AAAA / 纯 CNAME 链）、折叠（删除 CNAME、
  改写 A/AAAA owner）。
